# Code map — MIAOU

> **Généré par `build.py` à chaque build — ne pas éditer.**
> Index de repérage : fonctions/const top-level (JS) et sections (JS/CSS),
> avec numéros de ligne dans `src/`.

## src/js/utils.js

- **utils.js** (L3)
  - `escHtml()` — L11
- **Acks d'outils (journal client persistant des appels d'outils)** (L19)
  - `isAckRole()` — L22
  - `ackKindOf()` — L25
  - `ACK_COPY_FIELDS` — L38
  - `copyAckFields()` — L49
  - `placeCaretEnd()` — L59
  - `autoGrow()` — L69
- **Tokenisation / scoring (recherche mémoire)** (L77)
  - `STOPWORDS` — L79
  - `tokenize()` — L93
  - `scoreSummary()` — L99
- **Références de conversation dans le texte du modèle** (L110)
  - `CONV_REF_RE` — L117
  - `parseConvRefs()` — L119
- **Téléchargement côté client** (L129)
  - `downloadFile()` — L134
  - `LANG_TO_EXT` — L147
  - `langExt()` — L170
  - `b64ToBytes()` — L178
- **Agrégation MCP : nommage, namespaces, filtres (fonctions pures)** (L186)
  - `parseToolName()` — L191
  - `groupByNamespace()` — L203
  - `guessMcpTransport()` — L221
  - `validateMcpServerName()` — L231
  - `filterMcpTools()` — L245
  - `parseToolFilterList()` — L268
- **Horodatages des messages** (L275)
  - `SHOW_YEAR_AFTER_DAYS` — L277
  - `FR_DAYS_ABBR` — L279
  - `FR_DAYS_FULL` — L280
  - `FR_MONTHS_FULL` — L281
  - `_tsHHMM()` — L284
  - `_startOfDay()` — L290
  - `formatMessageTime()` — L301
  - `formatDateRelative()` — L325
  - `formatFullDateFr()` — L341
- **Reconstruction du payload API depuis currentThread** (L348)
  - `stampTs()` — L354
- **Export Markdown : traces d'appels d'outils** (L360)
  - `EXPORT_ARGS_MAX` — L363
  - `EXPORT_RESULT_MAX` — L364
  - `EXPORT_RESNAME_MAX` — L365
  - `_truncMd()` — L367
  - `_formatToolCallMd()` — L375
  - `formatToolAcksMd()` — L395
  - `_hashId9()` — L420
  - `messageTextForSummary()` — L434
  - `expandThread()` — L448
- **Parsing défensif du JSON de résumé** (L518)
  - `parseSummaryJSON()` — L521
- **Context inspector (brief B)** (L542)
  - `estimateTokens()` — L545
  - `IMAGE_TOKENS_ESTIMATE` — L553
  - `CONTEXT_WINDOW_WARN_RATIO` — L557
  - `buildContextManifest()` — L573

## src/js/storage.js

- **storage.js** (L1)
  - `SETTINGS_KEY` — L7
  - `CONV_KEY` — L8
  - `SUMMARIES_KEY` — L9
  - `SPACES_KEY` — L10
  - `ACTIVE_SPACE_KEY` — L11
  - `DEFAULT_SPACE_ID` — L12
  - `BUILD_CONFIG` — L25
  - `REQUIRE_API_KEY` — L27
  - `MAX_SUMMARIES` — L28
  - `BUILD_API_URL` — L29
  - `BUILD_API_MODEL` — L30
  - `BUILD_TS` — L31
  - `BUILD_DEFAULT_CONTEXT_WINDOW` — L37
  - `DEFAULT_SETTINGS` — L40
- **Réglages** (L58)
  - `loadSettingsRaw()` — L60
  - `loadSettings()` — L65
  - `saveSettings()` — L73
  - `contextWindowFor()` — L82
- **Serveurs API (multi-backends)** (L89)
  - `API_SERVERS_KEY` — L95
  - `ACTIVE_API_SERVER_KEY` — L96
  - `genApiServerId()` — L98
  - `migrateApiServersIfNeeded()` — L109
  - `saveApiServersRaw()` — L118
  - `loadApiServers()` — L123
  - `saveApiServers()` — L131
  - `normalizeApiServer()` — L135
  - `serverModelVisionEnabled()` — L164
  - `upsertApiServer()` — L170
  - `deleteApiServer()` — L179
  - `getApiServer()` — L185
  - `getActiveApiServerId()` — L189
  - `setActiveApiServerId()` — L193
  - `activeApiServer()` — L200
  - `activeApiConfig()` — L213
- **Serveurs MCP distants** (L222)
  - `MCP_SERVERS_KEY` — L231
  - `MCP_DEFAULT_TIMEOUT` — L233
  - `loadMcpServers()` — L235
  - `saveMcpServers()` — L242
  - `normalizeMcpServer()` — L249
  - `upsertMcpServer()` — L265
  - `deleteMcpServer()` — L274
  - `getMcpServer()` — L280
  - `listEnabledMcpServers()` — L284
- **Conversations** (L288)
  - `loadConversations()` — L291
  - `persistConversations()` — L296
  - `listAllConversations()` — L300
  - `loadConversation()` — L306
  - `saveConversation()` — L310
  - `deleteConversation()` — L318
  - `toggleConversationPin()` — L324
  - `hasSubstance()` — L337
- **Index des résumés (miaou-summaries)** (L349)
  - `loadSummaries()` — L354
  - `persistSummaries()` — L359
  - `getSummaryEntry()` — L363
  - `listSummaryEntries()` — L369
  - `saveSummary()` — L373
  - `suppressSummary()` — L383
  - `restoreSummary()` — L393
  - `deleteSummaryEntry()` — L403
  - `isSummaryCandidate()` — L410
  - `backfillCandidates()` — L415
- **Souvenirs utilisateur (miaou-memories)** (L420)
  - `MEMORIES_KEY` — L424
  - `genMemoryId()` — L426
  - `loadMemories()` — L428
  - `persistMemories()` — L433
  - `listMemoryEntries()` — L441
  - `saveMemory()` — L447
  - `editMemory()` — L455
  - `suppressMemory()` — L464
  - `restoreMemory()` — L470
  - `forgetMemory()` — L476
- **Espaces (miaou-spaces) — feature Spaces (lot C)** (L480)
  - `genSpaceId()` — L491
  - `loadSpaces()` — L496
  - `saveSpaces()` — L503
  - `normalizeSpace()` — L508
  - `upsertSpace()` — L519
  - `deleteSpaceEntry()` — L530
  - `getSpace()` — L537
  - `getActiveSpaceId()` — L541
  - `setActiveSpaceId()` — L545
  - `migrateSpacesIfNeeded()` — L553
  - `spaceConvIds()` — L578
- **Export / import complet des données (feature E)** (L586)
  - `EXPORT_KEYS` — L595
  - `buildExportPayload()` — L614
  - `validateImportPayload()` — L645

## src/js/resources.js

- **resources.js** (L1)
- **Helpers purs (QuickJS-testables)** (L8)
  - `classifyMime()` — L13
  - `humanSize()` — L21
  - `formatResourceDescriptor()` — L32
  - `generateResourceId()` — L38
- **Bibliothèque de fichiers d'espace (lot Cbis) — helpers purs** (L42)
  - `generateFileId()` — L47
  - `libraryRefFromId()` — L55
  - `LIBRARY_REF_RE` — L57
  - `parseLibraryRef()` — L60
  - `FILE_DESCRIPTION_MAX_CHARS` — L71
  - `capFileDescription()` — L72
  - `normalizeLibraryRecord()` — L80
  - `buildLibraryManifestBlock()` — L99
- **Pièces jointes (composer) — helpers purs (QuickJS-testables)** (L116)
  - `ATTACHMENT_TEXT_EXTENSIONS` — L122
  - `classifyAttachmentKind()` — L132
  - `allocateAttId()` — L146
- **Pièces jointes (composer) — envoi au modèle et politique de persistance** (L151)
  - `formatAttachmentDescriptor()` — L163
  - `formatTextAttachmentBlock()` — L173
  - `formatBinaryAttachmentDescriptor()` — L186
  - `buildAttachedMessageContent()` — L207
  - `prefixTextInContentParts()` — L230
  - `collapseAttachedMessageContent()` — L255
- **Codec base64 (hand-rolled, sans atob/btoa — QuickJS-testable)** (L265)
  - `_B64` — L267
  - `arrayBufferToBase64()` — L269
  - `base64ToArrayBuffer()` — L289
- **Codec UTF-8 (hand-rolled, sans TextEncoder/TextDecoder — QuickJS-testable)** (L309)
  - `utf8Encode()` — L311
  - `utf8Decode()` — L337
- **Partitionnement d'un résultat MCP (helper pur, QuickJS-testable)** (L362)
  - `extractResultParts()` — L370
- **Session cache** (L402)
  - `_resourceCache` — L408
  - `getCachedRecord()` — L410
  - `_cacheRecord()` — L411
  - `_uncacheRecord()` — L412
  - `clearResourceSessionCache()` — L413
  - `getCachedRecordByAttId()` — L420
  - `getCachedLibraryEntriesBySpace()` — L432
  - `loadSpaceLibrary()` — L446
- **Helpers de référence** (L455)
  - `_makeResourceRef()` — L459
  - `assembleToolResultForModel()` — L464
  - `resolveResourceRefs()` — L477
  - `resolveRecallImages()` — L497
- **IDB layer (navigateur uniquement — non QuickJS-testable)** (L508)
  - `_resourceDbPromise` — L512
  - `_persistenceRequested` — L513
  - `openResourceDB()` — L515
  - `putResource()` — L547
  - `getResource()` — L558
  - `getResourcesByConversation()` — L569
  - `getResourcesBySpace()` — L582
  - `deleteResource()` — L593
  - `deleteResourcesByConversation()` — L605
  - `requestPersistence()` — L621
  - `getAllResources()` — L632
  - `clearIdbStore()` — L646
- **Opérations haut-niveau** (L657)
  - `storeAttachment()` — L667
  - `storeLibraryFile()` — L694
  - `_storeBlock()` — L717
  - `internResourcesFromResult()` — L744
  - `loadConversationResources()` — L808
  - `makeResourcePresentBlock()` — L820

## src/js/skills.js

- **skills.js** (L1)
- **Helpers purs (QuickJS-testables)** (L25)
  - `SKILL_SLUG_MAX` — L27
  - `validateSkillSlug()` — L32
  - `parseSlashCommand()` — L49
  - `findSlashTriggers()` — L62
  - `bakeSkillMessage()` — L84
- **Cache mémoire (méta seulement : slug, name, description, enabled)** (L97)
  - `_skillsCache` — L101
  - `_skillMeta()` — L103
  - `setSkillsCache()` — L108
  - `upsertSkillCache()` — L114
  - `removeSkillCache()` — L121
  - `getSkillMeta()` — L126
  - `listAllSkillsCache()` — L131
  - `listEnabledSkills()` — L136
  - `getAutotriggerSkillsMeta()` — L144
  - `matchSkillCompletions()` — L152
- **Couche IDB (navigateur uniquement — non QuickJS-testable)** (L159)
  - `getAllSkillRecords()` — L164
  - `getSkillRecord()` — L176
  - `putSkill()` — L189
  - `deleteSkillDb()` — L210
  - `toggleSkillEnabled()` — L223
  - `getSkillContent()` — L234
  - `loadSkillsCache()` — L243

## src/js/tools.js

- **tools.js** (L1)
  - `summaryLight()` — L9
  - `BINARY_DOCTRINE` — L17
  - `ATTACHMENT_DOCTRINE` — L38
  - `WEB_DOCTRINE` — L56
  - `CONV_REF_DOCTRINE` — L98
  - `MEMORY_DOCTRINE` — L107
  - `FILES_DOCTRINE` — L138
  - `DOCS_DOCTRINE` — L164
  - `docsDoctrinePrompt()` — L173
  - `ROOT_SYSTEM_PROMPT` — L180
  - `SKILL_DOCTRINE_BASE` — L194
  - `SKILL_DOCTRINE_CONFIRM_ON` — L203
  - `SKILL_DOCTRINE_CONFIRM_OFF` — L213
  - `SKILL_DOCTRINE_TAIL` — L217
  - `INTENT_DOCTRINE` — L223
  - `_pendingToolAcks` — L234
  - `getPendingToolAcks()` — L235
  - `clearPendingToolAcks()` — L236
  - `_pendingImageInjections` — L247
  - `getPendingImageInjections()` — L248
  - `clearPendingImageInjections()` — L249
  - `updateLastPendingToolAck()` — L253
  - `_pendingToolBlocks` — L263
  - `getPendingToolBlocks()` — L264
  - `clearPendingToolBlocks()` — L265
  - `retainPendingToolBlocks()` — L268
  - `validateFilesPromoteArgs()` — L275
- **Registre MCP interne** (L282)
  - `TOOLS` — L285
- **ask_confirmation : primitif halting hors registre MCP** (L700)
  - `ASK_CONFIRMATION_DEF` — L707
- **Agrégation distante : cache de session + client MCP** (L729)
  - `MCP_PROTOCOL_VERSION` — L734
  - `REF_UNKNOWN_ERROR_CODE` — L740
  - `_remoteTools` — L742
  - `_remoteStatus` — L743
  - `getMcpStatus()` — L745
  - `remoteToolDefs()` — L748
  - `exposedTools()` — L760
- **Client JSON-RPC 2.0 sur transport streamable-http (cf. D4/D10)** (L767)
  - `_mcpRpcId` — L768
  - `mcpRpcAttempt()` — L775
  - `mcpReinitialize()` — L824
  - `mcpRpc()` — L838
  - `readSseJsonRpc()` — L855
  - `connectMcpServer()` — L885
  - `disconnectMcpServer()` — L915
  - `callRemoteTool()` — L932
- **Dispatcher MCP** (L963)
  - `callInternalTool()` — L965
  - `callTool()` — L1000
- **Hook d'inflation dispatcher (brief A, D6 — moitié client du lot D)** (L1030)
  - `_attachmentPushState` — L1036
  - `_pushStateKey()` — L1037
  - `isAttachmentPushed()` — L1038
  - `markAttachmentPushed()` — L1039
  - `clearAttachmentPushState()` — L1042
  - `_filePushState` — L1053
  - `_filePushStateKey()` — L1054
  - `isFilePushed()` — L1055
  - `markFilePushed()` — L1056
  - `toolDeclaresAttachmentInflation()` — L1064
  - `ATTACHMENT_REF_RE` — L1074
  - `FILE_REF_RE` — L1079
  - `anyToolDeclaresAttachmentInflation()` — L1087
  - `_declaresContentReadSignature()` — L1112
  - `findDocsInflationTool()` — L1125
  - `extractBinaryFileTextForDescription()` — L1150
  - `_resolveInflationRef()` — L1179
  - `callDocsInflatedRemoteTool()` — L1225
  - `_isRefUnknownError()` — L1256
  - `flattenToolResult()` — L1267
  - `toolIsHalting()` — L1280
  - `toolDefinitions()` — L1290
  - `toolsSystemPrompt()` — L1309
  - `intentDoctrinePrompt()` — L1318
  - `skillDoctrinePrompt()` — L1332

## src/js/api.js

- **api.js** (L1)
  - `MAX_TOURS` — L7
  - `_currentAbort` — L11
  - `NOTHINK_PARAMS` — L15
  - `_noThinkRejected` — L18
  - `_reasoningEffortRejected` — L25
  - `reasoningEffortRejectedKey()` — L26
  - `isReasoningEffortRejected()` — L27
  - `markReasoningEffortRejected()` — L28
  - `_visionRejected` — L35
  - `visionRejectedKey()` — L36
  - `isVisionRejected()` — L37
  - `markVisionRejected()` — L38
  - `messagesHaveImageParts()` — L43
  - `VISION_DEGRADED_NOTE` — L67
  - `degradeVisionMessages()` — L71
  - `injectVisionDegradedNote()` — L84
  - `TITLE_PROMPT` — L100
  - `SUMMARY_PROMPT` — L105
  - `FILE_DESCRIPTION_PROMPT` — L129
- **Appel non streamé, résultat exploité en interne (jamais affiché)** (L144)
  - `silentCompletion()` — L145
- **Parsing SSE** (L188)
  - `sseDataObject()` — L190
  - `parseSSELine()` — L203
  - `reasoningDelta()` — L215
  - `joinReasoning()` — L223
- **Streaming d'une complétion (un tour)** (L229)
  - `streamCompletion()` — L232
  - `abortStream()` — L372
- **Boucle complète d'un échange (injection + tool_calls)** (L376)
  - `runConversation()` — L386
- **Titrage automatique** (L564)
  - `generateTitle()` — L565
- **Génération d'un résumé** (L577)
  - `generateSummary()` — L578
- **Recherche / sélection des résumés pertinents** (L599)
  - `searchSummaries()` — L603
- **Liste des modèles exposés par l'API** (L621)
  - `fetchModels()` — L622

## src/js/ui.js

- **ui.js** (L1)
  - `highlightEnabled` — L7
  - `configured` — L8
  - `sending` — L9
  - `_confirmPending` — L10
- **Écran d'accueil (nouvelle conversation)** (L14)
  - `WELCOME_SCREENS` — L15
  - `showWelcome()` — L33
- **Rendu markdown / coloration** (L50)
  - `resolveConvRefs()` — L66
  - `renderMd()` — L78
  - `renderUserMd()` — L86
  - `highlightUnder()` — L91
  - `scrollBottom()` — L92
  - `modelName()` — L94
- **Construction d'un message** (L100)
  - `assistantHead()` — L105
  - `truncatedBannerHtml()` — L147
  - `buildMsg()` — L156
- **Bloc de raisonnement (thinking)** (L190)
  - `_reasonTimer` — L194
  - `_reasonPending` — L195
  - `renderReasoningNow()` — L199
  - `setReasoning()` — L214
  - `cancelReasoningRender()` — L229
  - `flushReasoning()` — L237
  - `toggleReasoning()` — L243
  - `decoratePre()` — L262
  - `downloadMsgMd()` — L303
  - `copyMsg()` — L327
- **Acks d'outils : table pilote (label + capacité d'annulation + icône)** (L352)
  - `ICON_MEMORY` — L357
  - `ICON_EDIT` — L358
  - `ICON_TRASH` — L359
  - `ICON_EYE` — L360
  - `ICON_LIST` — L361
  - `ICON_WRENCH` — L362
  - `ICON_CHEVRON_DOWN` — L363
  - `ICON_PACKAGE` — L364
  - `ICON_BOOK` — L365
  - `renderIntentTwoLevel()` — L372
  - `ACK_KINDS` — L406
- **Ressources IDB** (L491)
- **Bibliothèque de fichiers d'espace (lot Cbis)** (L564)
  - `ackLabel()` — L624
  - `buildToolAck()` — L629
  - `placeToolAck()` — L704
  - `renderThread()` — L749
  - `syncLastAssistantActions()` — L790
  - `syncConvDownloadBtn()` — L801
- **Streaming d'une réponse assistant** (L808)
  - `appendUserMessage()` — L809
  - `startAssistantMessage()` — L819
- **Patienteur animé** (L829)
  - `WAITER_WORDS` — L835
  - `_waiterRotate` — L840
  - `_waiterFade` — L841
  - `pickWaiterWord()` — L843
  - `startWaiter()` — L850
  - `stopWaiter()` — L866
  - `_streamTimer` — L875
  - `_streamPending` — L876
  - `streamInto()` — L878
  - `cancelStreamRender()` — L897
  - `resetAssistant()` — L902
  - `revealMsgTimestamp()` — L911
  - `finalizeAssistant()` — L921
- **Édition d'un message utilisateur** (L945)
  - `msgIndex()` — L948
  - `onEditMsg()` — L963
  - `enterEditMode()` — L969
  - `cancelEdit()` — L1021
  - `commitEdit()` — L1042
  - `showEditError()` — L1051
  - `clearEditError()` — L1055
- **Indicateur d'activité en arrière-plan** (L1060)
  - `_bgCount` — L1062
  - `bgActivityStart()` — L1063
  - `bgActivityEnd()` — L1068
  - `bgActivityLabel()` — L1072
- **Sidebar / sections temporelles** (L1076)
  - `sectionFor()` — L1077
  - `relativeWhen()` — L1090
  - `convSearchFilter` — L1109
  - `searchConversations()` — L1115
  - `onConvSearch()` — L1151
  - `clearConvSearch()` — L1158
- **Suppression en deux temps (« armer puis confirmer »)** (L1171)
  - `ARM_DELETE_MS` — L1179
  - `armThenRun()` — L1181
  - `onConvDel()` — L1200
  - `PIN_SVG` — L1205
  - `convItemEl()` — L1207
  - `sectionEl()` — L1225
  - `renderConvList()` — L1232
  - `isMobileLayout()` — L1260
  - `closeSidebarMobile()` — L1262
  - `toggleSidebar()` — L1268
  - `initVisualViewport()` — L1280
- **Redimensionnement de la sidebar (drag du bord droit)** (L1291)
  - `SIDEBAR_MIN` — L1295
  - `SIDEBAR_MAX` — L1296
  - `_sidebarW` — L1297
  - `applySidebarWidth()` — L1299
  - `initSidebarResize()` — L1305
  - `setTitle()` — L1340
  - `apiKeyFieldHint()` — L1349
- **État configuré / non configuré** (L1355)
  - `syncConfigured()` — L1356
  - `setSending()` — L1380
  - `setComposerStreaming()` — L1397
  - `setConnDot()` — L1403
  - `setConfirmPending()` — L1413
  - `dismissConfirmation()` — L1429
- **Composer** (L1436)
  - `onComposerKey()` — L1437
- **Pièces jointes (composer) : drag & drop + chips** (L1450)
  - `onComposerDragOver()` — L1452
  - `onComposerDragLeave()` — L1457
  - `onComposerDrop()` — L1463
  - `onComposerPaste()` — L1477
  - `attIconSvg()` — L1494
  - `attChipHtml()` — L1505
  - `promoteAttachmentToLibrary()` — L1535
  - `resolveAttachmentThumb()` — L1560
  - `renderComposerAttachments()` — L1570
  - `renderMsgAttachments()` — L1582
- **Dropdown modèle (liste via l'API)** (L1589)
  - `_models` — L1593
  - `openApiModelMenu()` — L1595
  - `renderApiModelOptions()` — L1617
  - `onApiModelInput()` — L1633
- **Sélecteur de modèle du composer** (L1661)
  - `_modelsCache` — L1664
  - `_modelsCacheUrl` — L1665
  - `loadModelsCached()` — L1667
  - `syncModelUI()` — L1681
  - `toggleComposerModelMenu()` — L1692
  - `renderComposerModelOptions()` — L1702
  - `pickComposerModel()` — L1715
- **Sélecteur de niveau de raisonnement du composer** (L1720)
  - `REASONING_EFFORT_OPTIONS` — L1727
  - `syncReasoningUI()` — L1735
  - `toggleComposerReasoningMenu()` — L1753
  - `renderComposerReasoningOptions()` — L1761
  - `pickComposerReasoningEffort()` — L1774
  - `toggleSettingsReasoningMenu()` — L1783
  - `renderSettingsReasoningOptions()` — L1791
  - `pickSettingsReasoningEffort()` — L1804
  - `syncSettingsReasoningLabel()` — L1814
- **Settings drawer** (L1821)
  - `toggleSettingsCat()` — L1826
  - `settingsFormDirty()` — L1844
  - `updateSettingsDirty()` — L1864
  - `openSettings()` — L1869
  - `closeSettings()` — L1890
- **Catégorie « Données » : export / import complet (feature E)** (L1900)
  - `resetImportDataUI()` — L1908
  - `showImportDataError()` — L1915
  - `renderImportSummary()` — L1924
  - `SUMMARY_INJECTION_HINTS` — L1942
  - `pendingSummaryInjectionMode` — L1948
  - `setSummaryInjectionModeUI()` — L1949
  - `selectSummaryInjectionMode()` — L1957
- **Thème** (L1959)
  - `THEME_HINTS` — L1960
  - `pendingTheme` — L1966
  - `applyTheme()` — L1971
  - `setThemeUI()` — L1991
  - `selectTheme()` — L1999
  - `onToggleHighlight()` — L2005
- **Bannière résumés (mode « proposer »)** (L2010)
  - `_bannerHandlers` — L2011
  - `showSummaryBanner()` — L2012
  - `hideSummaryBanner()` — L2040
  - `summaryBanner()` — L2045
- **Drawer combiné Résumés / Souvenirs** (L2051)
  - `openSummaryDrawer()` — L2052
  - `openMemoryDrawer()` — L2057
  - `closeSummaryDrawer()` — L2058
- **Inspecteur de contexte (brief B)** (L2063)
  - `CTX_PALETTE` — L2067
  - `effectiveContextManifest()` — L2080
  - `syncContextCounter()` — L2086
  - `openContextInspector()` — L2112
  - `closeContextInspector()` — L2117
  - `renderContextInspector()` — L2122
  - `switchMemoryTab()` — L2163
  - `renderSummaryList()` — L2175
  - `deleteSummaryItem()` — L2223
  - `toggleSummaryExpand()` — L2225
  - `restoreSummaryItem()` — L2236
- **Panneau des outils** (L2266)
  - `openTools()` — L2267
  - `closeTools()` — L2272
  - `renderToolsList()` — L2281
  - `buildToolItem()` — L2338
- **Spaces / « Espaces » (sélecteur sidebar + écran, lot C, brief D5)** (L2370)
  - `syncSpaceUI()` — L2378
  - `toggleSpaceMenu()` — L2389
  - `renderSpaceMenu()` — L2397
  - `pickSpace()` — L2430
  - `createSpaceAndOpen()` — L2448
- **Écran Space (sous-drawer, pattern MCP)** (L2457)
  - `_spaceScreenId` — L2458
  - `openSpaceScreen()` — L2460
  - `closeSpaceScreen()` — L2481
  - `onSpaceFormInput()` — L2487
  - `onSaveSpaceScreen()` — L2492
  - `syncSpaceDeleteLabel()` — L2517
  - `onDeleteSpaceScreen()` — L2531
- **Sous-drawer « Serveurs MCP » (cartes éditables, cf. D3)** (L2562)
  - `openMcpServers()` — L2563
  - `closeMcpServers()` — L2568
  - `renderMcpServersIfOpen()` — L2572
  - `renderMcpServers()` — L2576
  - `addMcpServerCard()` — L2593
- **Helpers partagés des cartes de configuration (MCP / API / skills)** (L2604)
  - `showCardError()` — L2610
  - `cfgField()` — L2615
  - `cfgToggle()` — L2632
  - `cfgToggleRow()` — L2646
  - `cfgErrEl()` — L2658
  - `cfgPillSelect()` — L2672
  - `buildMcpCard()` — L2719
- **SECTION VUE** (L2724)
- **SECTION ÉDITION** (L2783)
- **Sous-drawer « Serveurs API » (cartes éditables, même pattern que MCP)** (L2856)
  - `openApiServers()` — L2859
  - `closeApiServers()` — L2864
  - `renderApiServersIfOpen()` — L2868
  - `syncActiveApiServerUI()` — L2875
  - `renderApiServers()` — L2901
  - `addApiServerCard()` — L2917
  - `buildApiCard()` — L2925
- **SECTION VUE** (L2930)
- **SECTION ÉDITION** (L2971)
- **Skills : drawer de gestion** (L3048)
  - `openSkills()` — L3051
  - `closeSkills()` — L3056
  - `syncSkillHintUI()` — L3063
  - `renderSkills()` — L3068
  - `addSkillCard()` — L3084
  - `buildSkillCard()` — L3092
- **SECTION VUE** (L3097)
- **SECTION ÉDITION** (L3131)
  - `enterSkillEdit()` — L3189
- **Autocomplétion des skills (slash-commande)** (L3197)
  - `_composerAc` — L3205
  - `onComposerInput()` — L3207
  - `updateSkillAutocomplete()` — L3221
  - `renderSkillAutocomplete()` — L3235
  - `hideSkillAutocomplete()` — L3260
  - `skillAutocompleteOpen()` — L3268
  - `moveSkillAcSelection()` — L3273
  - `acceptSkillAcSelection()` — L3290
  - `pickSkillCompletion()` — L3303
  - `showComposerSkillError()` — L3317
  - `clearComposerSkillError()` — L3321
- **Cascade de rendu des blocs NON-text d'un résultat d'outil distant (D8)** (L3326)
  - `placeToolBlocks()` — L3330
  - `renderToolBlock()` — L3341
  - `renderResourceText()` — L3371
  - `renderBinaryBlock()` — L3389
  - `presentResourceFromChip()` — L3412
  - `mimeToLang()` — L3429
  - `setMemItemLoading()` — L3442
- **Souvenirs utilisateur (onglet Souvenirs du drawer combiné = profile ;** (L3450)
  - `renderMemoryList()` — L3458
  - `addMemoryEntry()` — L3521
  - `deleteMemoryEntry()` — L3532
  - `restoreMemoryEntry()` — L3533
  - `forgetMemoryEntry()` — L3534
  - `promoteMemoryEntry()` — L3539
- **Bibliothèque de fichiers d'espace (D6, lot Cbis)** (L3548)
  - `renderSpaceFilesList()` — L3554
  - `setFileDescriptionStatus()` — L3591
  - `onRegenerateFileDescription()` — L3609
  - `onSpaceFilesUploadClick()` — L3619
  - `onSpaceFilesSelected()` — L3627
  - `onDeleteSpaceFile()` — L3654
  - `startEditMemoryEntry()` — L3662
  - `cancelMemoryEntryEdit()` — L3673
  - `saveMemoryEntryEdit()` — L3682
- **Confirmation inline (cartes dans le thread)** (L3691)
  - `_proposalMap` — L3695
  - `clearMemoryProposals()` — L3699
  - `showConfirmation()` — L3706
  - `acceptProposal()` — L3728
  - `rejectProposal()` — L3736
  - `_removeProposalCard()` — L3744

## src/js/main.js

- **main.js** (L1)
- **Logo : source unique (favicon + sidebar)** (L10)
  - `LOGO_SRC` — L13
  - `applyLogo()` — L30
- **État de session** (L36)
  - `currentConvId` — L37
  - `activeSpaceId` — L38
  - `currentThread` — L39
  - `needTitle` — L40
  - `titleBefore` — L41
  - `currentConvModel` — L42
  - `currentConvReasoningEffort` — L43
  - `pendingAttachments` — L44
  - `attachIngestInFlight` — L45
  - `_lastContextManifest` — L46
  - `_lastContextManifestMidTurn` — L47
- **Résumé sur inactivité** (L49)
  - `IDLE_SUMMARY_MS` — L53
  - `_idleSummaryTimer` — L54
  - `armIdleSummaryTimer()` — L56
  - `activeModel()` — L68
  - `setConvModel()` — L77
  - `activeReasoningEffort()` — L92
  - `setConvReasoningEffort()` — L98
- **Construction du message système (un seul, concaténé)** (L110)
  - `buildSummaryBlock()` — L111
  - `buildMemoryEntriesBlock()` — L126
  - `contextBlockParts()` — L137
  - `buildContextBlock()` — L159
  - `buildSkillsContextBlock()` — L180
  - `resolveUserSystemPrompt()` — L195
  - `systemMessageParts()` — L214
  - `buildSystemMessage()` — L234
  - `computeContextManifestNow()` — L246
  - `recomputeLastContextManifest()` — L267
- **Navigation entre conversations** (L275)
  - `openConversation()` — L276
  - `resetToEmpty()` — L311
  - `selectConv()` — L329
  - `newConversation()` — L338
  - `togglePin()` — L347
  - `downloadConvMd()` — L357
  - `undoToolAck()` — L399
  - `deleteConv()` — L413
  - `ensureConversation()` — L423
  - `persistCurrent()` — L434
- **Titre éditable** (L463)
  - `wireTitleEditing()` — L464
  - `onTitleKey()` — L473
  - `onTitleBlur()` — L477
- **Réglages** (L495)
  - `onSaveSettings()` — L496
  - `prefetchModels()` — L526
- **Serveurs MCP distants : orchestration** (L531)
  - `reconnectMcpServers()` — L535
  - `onSaveMcpCard()` — L544
  - `onDeleteMcpCard()` — L576
- **Serveurs API : persistance + activation (orchestration depuis le drawer)** (L581)
  - `onSaveApiCard()` — L585
  - `onDeleteApiCard()` — L621
  - `onUseApiServer()` — L637
- **Skills : persistance (orchestration depuis le drawer de gestion)** (L650)
  - `onSaveSkillCard()` — L654
  - `onDeleteSkillCard()` — L676
  - `onToggleSkill()` — L682
- **Export / import complet des données (feature E)** (L687)
  - `snapshotLocalStorageForExport()` — L695
  - `exportAllData()` — L708
  - `onImportDataClick()` — L721
  - `onImportFileSelected()` — L730
  - `applyImportedData()` — L751
- **Pièces jointes (composer)** (L771)
  - `ATTACHMENT_MAX_BYTES` — L781
  - `ATTACHMENT_IMAGE_MAX_EDGE` — L782
  - `ATTACHMENT_IMAGE_JPEG_QUALITY` — L783
  - `ATTACHMENT_TEXT_MAX_BYTES` — L784
  - `ATTACHMENT_MAX_IMAGES` — L785
  - `downscaleImageFile()` — L793
  - `readFileAsText()` — L820
  - `readFileAsArrayBuffer()` — L830
  - `showComposerAttachError()` — L842
  - `clearComposerAttachError()` — L846
  - `ingestAttachmentFile()` — L859
  - `showSpaceFilesError()` — L920
  - `clearSpaceFilesError()` — L924
  - `ingestLibraryFile()` — L937
  - `persistAttSeq()` — L975
  - `handleAttachFiles()` — L990
  - `onAttachClick()` — L1008
  - `onAttachFilesSelected()` — L1014
  - `removeComposerAttachment()` — L1022
- **Flux d'envoi** (L1027)
  - `onSendBtn()` — L1029
  - `resolveSend()` — L1048
  - `sendMessage()` — L1075
  - `buildOutgoingContentForAttachments()` — L1124
  - `sendUserText()` — L1159
  - `runGenerationFromCurrentThread()` — L1191
  - `editUserMessage()` — L1221
  - `regenerateResponse()` — L1250
  - `continueTruncated()` — L1266
  - `rewriteAttachedUserMessage()` — L1294
  - `dispatchSend()` — L1301
- **Mécanique réutilisable : tâche LLM « en arrière-plan »** (L1649)
  - `runBackgroundTask()` — L1653
- **Titrage automatique (après la première réponse)** (L1665)
  - `applyGeneratedTitle()` — L1666
  - `maybeTitle()` — L1673
  - `regenerateTitle()` — L1689
  - `setTitleEditable()` — L1702
- **Résumé / mots-clés à la sortie d'une conversation** (L1708)
  - `summarizeIfNeeded()` — L1709
- **Description de fichier de bibliothèque d'espace (D7, lot Cbis)** (L1727)
  - `FILE_DESCRIPTION_EXTRACT_MAX_CHARS` — L1734
  - `describeFileIfNeeded()` — L1748
- **Backfill modèle : attribue le modèle courant aux réponses sans modèle** (L1780)
  - `backfillMessageModels()` — L1781
- **Backfill au démarrage (séquentiel, indicateur continu)** (L1798)
  - `runBackfill()` — L1799
- **Init** (L1826)
  - `init()` — L1827

## src/css/base.css

- **base.css** (L1)
- **App shell** (L65)

## src/css/sidebar.css

- **sidebar.css** (L1)
- **Sidebar** (L6)

## src/css/chat.css

- **chat.css** (L1)
- **Main** (L7)
- **Messages** (L168)

## src/css/composer.css

- **composer.css** (L1)
- **Composer** (L6)
- **Pièces jointes (composer)** (L302)

## src/css/drawers.css

- **drawers.css** (L1)
- **Settings drawer** (L7)
- **Drawer réglages : catégories repliables** (L277)

## src/css/tools.css

- **tools.css** (L1)
- **Ack d'outils (journal client : écritures mémoire + lectures d'historique)** (L11)
- **Écran d'accueil (nouvelle conversation)** (L78)
- **Cartes de configuration (partagé MCP / API / skills)** (L161)
- **Serveurs MCP : particularités (D3)** (L222)
- **Serveurs API : particularités** (L228)
- **Skills : particularités du drawer de gestion** (L240)
- **Composer : autocomplétion des skills + erreur slash** (L259)
- **Blocs non-text d'un résultat d'outil distant (cascade D8)** (L293)

## src/css/responsive.css

- **responsive.css** (L1)
- **Responsive** (L6)
- **Mobile : sidebar overlay (< 768px)** (L40)

## src/css/theme-light.css

- **theme-light.css** (L1)
- **Thème clair** (L7)
